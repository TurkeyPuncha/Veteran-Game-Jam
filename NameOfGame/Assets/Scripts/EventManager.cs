...
static internal class EventManager
{
    ...
    static public void Fire<TEvent>(TEvent evt) where TEvent : TEventBase
    {
        ...
        if (!m_eventTable.ContainsKey(eventType))
        {
            Debug.Log("No event for ".ConcatString(eventType.ToString()));
            return;
        }
        ...
        {
            throw new FireEventException("Fire message \"{0}\" failed. Could not find associated delegate.".ConcatString(eventType.ToString()));
        }
    }
}